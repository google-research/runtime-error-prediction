...
from experimental.compressive_ipagnn import common_modules
...
class SkipEmbedder(nn.Module):
  ...
  def apply(self, node_embeddings, max_steps,
    ...
    embedder = SkipEmbedderSingleSource.shared(
...
class SkipEmbedderSingleSource(nn.Module):
  ...
  def apply(self, from_node_index, node_embeddings, max_steps,
    ...
    execute_lstm = StackedRNNCell.shared(cells=execute_cells,
...
class SkipEncoderLineByLine(nn.Module):
  ...
  def apply(self,
    ...
    """
    hidden_size = config.model.hidden_size
    skip_embedder_layer_norm = nn.LayerNorm.shared(
    ...
    cells = create_lstm_cells(config.model.rnn_cell.layers)
    lstm = StackedRNNCell.partial(cells=cells, name='skip_encoder_rnn')
...
def make_mask_maker(config):
  ...
  if mask_maker_kind == 'default':
    return MaskMaker.shared(config=config, name='mask_maker')
  elif mask_maker_kind == 'no-skip':
    return MaskMakerNoSkip.shared(config=config, name='mask_maker')
  elif mask_maker_kind == 'line-by-line':
    return MaskMakerLineByLine.shared(config=config, name='mask_maker')
  else:
...
class SkipDecider(nn.Module):
  ...
  def apply(self, hidden_states, skip_embeddings, skip_mask, config):
    decider = SkipDeciderSingleSource.shared(config=config)
    # leaves(hidden_states).shape: num_nodes, hidden_size
...
class SkipDeciderSingleSource(nn.Module):
  ...
  def apply(self, hidden_states, skip_embeddings, skip_mask, config):
    ...
    hidden_size = config.model.hidden_size
    key_dense = nn.Dense.shared(
        name='key_dense',
        features=hidden_size,
        kernel_init=nn.initializers.xavier_uniform(),
        bias_init=nn.initializers.normal(stddev=1e-6))
    query_dense = nn.Dense.shared(
        name='query_dense',
        features=hidden_size,
        kernel_init=nn.initializers.xavier_uniform(),
        bias_init=nn.initializers.normal(stddev=1e-6))
    logit_dense = nn.Dense.shared(
        name='logit_dense',
        features=1,
        kernel_init=nn.initializers.xavier_uniform(),
        bias_init=nn.initializers.normal(stddev=1e-6))
...
class SkipIPAGNNSingle(nn.Module):
  ...
  def apply(self, inputs, info, config):
    ...
    # Create modules:
    node_embedder = NodeEmbedder.shared(info=info, config=config,
                                        name='node_embedder')
    # Make skip embedder.
    max_skip_steps = (config.model.skip_encoder.max_skip
                      or max_steps)
    skip_embedder = SkipEmbedder.shared(
    ...
    skip_decider = SkipDecider.shared(config=config, name='skip_decider')
    execute_cells = create_lstm_cells(config.model.rnn_cell.layers)
    skip_executor = SkipExecutor.shared(
        execute_cells=execute_cells, config=config,
        name='skip_executor')
    branch_decider = BranchDecider.shared(name='branch_decider')
    aggregator = Aggregator.shared(
        true_indexes=true_indexes, false_indexes=false_indexes, config=config,
        name='aggregator')
    decoder = Decoder.shared(vocab_size=output_token_vocabulary_size,
                             name='decoder')
...
class SkipIPAGNN(nn.Module):
  ...
  def apply(self, inputs, info, config, train=False, cache=None):
    ipagnn = SkipIPAGNNSingle.shared(info=info, config=config, name='ipagnn')
