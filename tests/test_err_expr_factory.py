...
class TestErrorFactory:
    ...
    def setup(self):
        ...
        self.test_lst_var_assign = (
            self.test_lst_var_name + "=" + str(self.test_lst_var_val) + "\n"
        )

    def test_get_zero_perturb_expression(self):

        expr, is_err_present = self.error_factory.add_err(
            "zero_err", self.test_var_name, self.test_var_val
        )
    ...
    def test_get_assert_perturb_expression(self):
        expr, is_err_present = self.error_factory.add_err(
            "assert_err", self.test_var_name, self.test_var_val
        )
        expr = self.test_var_assign + expr
        while not is_err_present:
            subprocess_call = subprocess.call(
    ...
    def test_get_not_subscriptable_perturb_expression(self):
        expr, is_err_present = self.error_factory.add_err(
            "not_subscriptable_err", self.test_var_name, self.test_var_val
        )
        expr = self.test_var_assign + expr
        while not is_err_present:
            subprocess_call = subprocess.call(
        ...
        with pytest.raises(subprocess.CalledProcessError) as exc:
            try:
                ...
                if (
                    "TypeError" in exception.output
                    and "not support item assignment" in exception.output
                ):
                    raise exception

    def test_get_index_range_perturb_expression(self):
        expr, is_err_present = self.error_factory.add_err(
            "idx_out_range_err", self.test_lst_var_name, self.test_lst_var_val
        )
        ...
        with pytest.raises(subprocess.CalledProcessError) as exc:
            try:
                ...
                if (
                    "IndexError" in exception.output
                    and "index out of range" in exception.output
                ):
                    raise exception

    def test_get_math_domain_perturb_expression(self):
        expr, is_err_present = self.error_factory.add_err(
            "math_domain_err", self.test_var_name, self.test_var_val
        )
        expr = self.test_var_assign + expr
        ...
        with pytest.raises(subprocess.CalledProcessError) as exc:
            try:
                ...
                if (
                    "ValueError" in exception.output
                    and "math domain error" in exception.output
                ):
                    raise exception

    def test_get_int_not_iterable_perturb_expression(self):
        expr, is_err_present = self.error_factory.add_err(
            "not_iterable_err", self.test_var_name, self.test_var_val
        )
        expr = self.test_var_assign + expr
        ...
        with pytest.raises(subprocess.CalledProcessError) as exc:
            try:
                ...
                if (
                    "TypeError" in exception.output
                    and "object is not iterable" in exception.output
                ):
                    raise exception
