...
class ErrorFactory:
    ...
    def get_math_domain_perturb_expression(self, perturb_var, perturb_val):
        ...
        if is_math_domain_err:
            assign_var = get_random_list_sample(self.VARIABLE_NAMES, 1)[0]
            if perturb_val>=0:
                random_ass = str(-1*int(get_random_float(*self.NUM_RANGE, size=1)[0])) + "*" + perturb_var
            else:
                random_ass = str(int(get_random_float(*self.NUM_RANGE, size=1)[0])) + "*" + perturb_var
            return (
                "import math;"
                + assign_var
                + "="
                + "math.sqrt("+str(random_ass)+")",
                is_math_domain_err,
            )
        else:
            assign_var = get_random_list_sample(self.VARIABLE_NAMES, 1)[0]
