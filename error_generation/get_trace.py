...
def run_for_errors(python_filepath, data_path, trace_path, stdin_file, stdout_file, stderr_file, processed_suffix = 'processed'):
  ...
  try:
    subprocess_call = subprocess.check_call(['python', '-c', python_source], stdin=open(stdin_file, 'rb'), stdout=open(stdout_file, 'wb'), stderr=open(stderr_file, 'wb'))
  except Exception as e:
    return False
  postprocess_and_save(data_path, 3, processed_suffix)
  return True
