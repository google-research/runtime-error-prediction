...
def evaluate_cfg(executor, cfg, mod=None, initial_values=None, timeout=None):
  ...
  while block:
    ...
    block, values = evaluate_until_next_basic_block(
        executor, block, mod=mod, values=values)
    blocks_evaluated += 1
  return values
...
def evaluate_basic_block(executor, basic_block, mod, values):
  ...
  for index, node in enumerate(nodes):
    ...
    python_source = astunparse.unparse(ast_node)
