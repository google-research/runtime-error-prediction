...
class StackedLSTMModel(nn.Module):
    ...
    def __call__(
        ...
        lstm = SimpleLSTM()
        logits = nn.Dense(features=NUM_CLASSES)

        x = inputs.astype("int32")

        program_line_embeddings = embed(x)
        ...
        (c, h), forward_outputs = lstm(initial_state, program_line_embeddings)
        output = logits(forward_outputs[:, -1, :])
        return output
